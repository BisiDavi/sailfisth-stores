...
import { Container, Row, Col, Breadcrumb, BreadcrumbItem, Button, Input, FormGroup, Label, Form, Collapse, Table } from 'reactstrap'
...
const Checkout = () => {
    ...
    const subTotal = cart.reduce((sum, { price, quantity }) => sum + price * quantity, 0) // Count price of cart items together
    ...
    React.useEffect(() => {
        !formInputs.shipping && setFormInputs({ ...formInputs, shipping: defaultShipping, payment: defaultPayment, cart: JSON.parse(localStorage.getItem('cart')) }) // If checkout inputs not set then set default shipping, default payment and cart items to checkout context
    }, [])

    const total = formInputs.shipping ? subTotal + formInputs.shipping[2] : subTotal + defaultShipping[2] // Total price including shipping fee. If checkout inputs not set then use default shipping price
    return (
        <React.Fragment>
            <section className="hero py-6">
                <Container>
                    ...
                    <div className="hero-content">
                        ...
                        <div>
                            <p className="lead text-muted">Are you a returning customer? <Link href="/customer-login"><a>Login</a></Link>.</p>
                        </div>
                    </div>
                </Container>
            </section>
            <Container className="pb-6">
                <Row>
                                        ...
                                        <Row>
                                            {block.inputs.map((input, index) =>
                                                <React.Fragment key={index}>
                                                    ...
                                                    {/* TEXT INPUT */}
                                                    {input.type === "text" &&
                                                        <FormGroup key={index} className="col-md-6 mb-4">
                                                            <Label className="form-label" for={input.name}>{input.label}</Label>
                                                            <Input className="form-control-underlined pl-0" type={input.type} name={input.name} placeholder={input.placeholder ? input.placeholder : ''} id={input.name} value={formInputs[input.name] || ''} onChange={(e) => onChange(e)} />
                                                        </FormGroup>
                                                    }
                                                    {/* END TEXT INPUT */}
                    ...
                    <Col lg="5">
                            ...
                            <tbody>
                                ...
                                {cart.map(item =>
                                    <tr key={item.name} className="text-sm">
                                        <th className="py-4 font-weight-normal text-muted">{item.name} <span>x {item.quantity}</span></th>
                                        <td className="py-4 text-right text-muted">${(item.price * item.quantity).toFixed(2)}</td>
                                    </tr>
                                )}
                                {/* END CART ITEMS */}
                                ...
                                {/* SUB TOTAL */}
                                <tr>
                                    <th className="py-4 text-uppercase font-weight-normal text-sm align-bottom">Order Subtotal </th>
                                ...
                                <tr>
                                    <th className="py-5 border-dark" colSpan="2">
                                        ...
                                        {inputs.shipping.map((input, index) =>
                                            <FormGroup key={index} className="mb-1 ml-4">
                                                <div className="custom-control custom-radio">
                                                    <input className="custom-control-input" id={input.id} type="radio" name={input.name}
                                                        onChange={(e) => onRadioChange(e, input.label, input.price)} // On change func
                                                    ...
                                                    />
                                                    <label className="custom-control-label text-sm font-weight-normal" htmlFor={input.id}>{input.label}</label>
                                                </div>
                                            </FormGroup>
                                        )}
                                    </th>
                                ...
                                {/* TOTAL */}
                                <tr>
                                    <th className="py-4 text-uppercase font-weight-normal border-dark align-bottom">Total</th>
                                    <td className="py-4 text-right h3 font-weight-normal border-dark">${total.toFixed(2)}</td>
                                </tr>
                                {/* END TOTAL */}
                                ...
                                {inputs.payment.map((input, index) =>
                                    <tr key={index}>
                                        <th className={`${index === 0 ? 'pt-5 pb-4 border-dark' : ''} ${index === inputs.payment.length - 1 ? 'pt-4 pb-5' : ''} ${index !== 0 && index < inputs.payment.length - 1 ? 'py-4' : ''}`} colSpan="2">
                                            <div className="custom-control custom-radio">
                                                <input className="custom-control-input" id={input.id} type="radio" name={input.name}
                                            ...
                                            <Collapse isOpen={formInputs.payment && formInputs.payment[0] === input.id}>
                                                <div className="pt-4">
                        ...
                        <p className="text-muted text-sm">Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our <a href="#">privacy policy</a>.</p>
                        <p className="text-muted text-sm mb-5">I have read and agree to the website <a href="#">terms and conditions</a>. * </p>

                        {/* PLACE ORDER */}
                        <Link href="/checkout-confirmed" passHref><Button color="dark" size="lg" block className="mb-5">Place order</Button></Link>
                        {/* END PLACE ORDER */}
