...
const CheckoutConfirmed = () => {
    ...
    const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] // DEMO ONLY - English month names
    const month = months[today.getMonth()] // Demo month
    const year = today.getFullYear() // Demo year

    // Remove on production
    const finalPrice = formInputs.cart ? formInputs.shipping[2] + formInputs.cart.reduce((sum, { price, quantity }) => sum + price * quantity, 0) : 450
        ...
        <React.Fragment>
                ...
                <Container>
                    ...
                    <div className="hero-content">
                        ...
                        <Alert color="success" className="d-flex align-items-center">
                            <Icon icon="checked-circle-1" className="w-3rem h-3rem svg-icon-light flex-shrink-0 mr-3" />Your order is confirmed
                        </Alert>
                    </div>
                </Container>
            </section>
            <section className="pb-6">
                <Container>
                    ...
                    <p className="lead mb-5">Your order hasn't shipped yet but we will send you ane email when it does.</p>
                    <p className="mb-6">
                    ...
                    <div className="p-5 bg-gray-100">
                        <Row>
                            {order.map(item =>
                                ...
                                <div className="text-sm text-uppercase text-muted mb-3">{item.label}</div>
