...
import { Container, Breadcrumb, BreadcrumbItem, Row, Col, Button, Form, Label, Input, FormGroup } from 'reactstrap'
...
const PostPage = ({ postData }) => {
    ...
    {/* Next.js statically generates the post content, but we change just some parts for the sake of simplicity - see them in postData. The rest is dummy text from the post variable stored in this file. */ }
                    ...
                    <Row>
                        <Col xl="8" lg="10" className="mx-auto">
                            <Breadcrumb className="pl-0" listClassName="justify-content-center">
                                ...
                                <BreadcrumbItem active>{postData.name}</BreadcrumbItem>
                            </Breadcrumb>
                            <div className="hero-content">
                                <h1 className="hero-heading">{postData.name}</h1>
                                <div>
                                    <p className="text-muted mb-0">
                                        <a href={post.author.link}><img src={post.author.avatar} alt="" className="w-2rem rounded-circle mr-2" /></a> By <a href="#" className="text-muted font-weight-bold">{post.author.name}</a> <span className="mx-1">|</span> {postData.date} in <Link href="/blog"><a className="font-weight-bold">{postData.category[0]}</a></Link> <span className="mx-1">|</span> <a href="#comments" className="text-muted" data-smooth-scroll>{post.comments.length} comments</a>
                                    </p>
                                </div>
                            </div>
                        </Col>
            ...
            <Container>
                <Row>
                    <Col xl="11" className="mx-auto">
                        <img className="img-fluid mb-5" src={postData.img} alt="" />
                    </Col>
                </Row>
                <Row>
