...
const FormCheckout = ({ step, next, prev }) => {
    ...
    const [multiCollapse, setMultiCollapse] = React.useState(formInputs['Payment Method'] ? Object.keys(formInputs['Payment Method'])[0] : {}) // Set default first payment method
    ...
    const onChange = (e, groupName, tabName) => {
        ...
        groupName ? // If group value present set group value
            ...
            formInputs[groupName] ? // If group input value present set update
                setFormInputs({ ...formInputs, [groupName]: { [tabName]: { ...formInputs[groupName][tabName], [e.target.name]: value } } })
                : // else set values
                setFormInputs({ ...formInputs, [groupName]: { [tabName]: { [e.target.name]: value } } })
    ...
    const onRadioChange = (e, input, groupName, tabName) => {
        ...
        groupName ? // If group value present set group value
            setFormInputs({ ...formInputs, [groupName]: { [tabName]: [value, input.label] } })
            : // else set single input
            setFormInputs({ ...formInputs, [e.target.name]: [value, input.label, input.price] })
    }
    return (
        <Form>
            {data.map(block => {
                return (
                    block.step === step ? // Checkout step check
                                            ...
                                            {input.type === "radio" &&
                                                <Col md="6" key={index}>
                                                    <div className="card-radio mb-4">
                                                        ...
                                                        <input className="card-radio-input" type={input.type} name={input.name} id={input.id}
                                                        ...
                                                        <Card>
                                                            ...
                                                            <CardBody>
                                                                {input.price && <h6 className="mb-3">${input.price.toFixed(2)}</h6>}
                                                                <p className="text-muted text-sm card-text">{input.text}</p>
                                                            </CardBody>
                                                        </Card>
                                                    </div>
                                                </Col>
                                        ...
                                        return (
                                            <Card key={tab.name} className="border-0 shadow mb-3">
                                                <CardHeader className="accordion-header">
                                                    <a href="#" className={`accordion-link ${!isCollapse ? 'collapsed' : ''}`} onClick={(e) => toggleCollapse(e, tab.name)} aria-expanded={isCollapse}>{tab.name}</a>
                                                </CardHeader>
                                                                ...
                                                                input.type === "radio" ?
                                                                    <Col key={input.name}>
                                                                        <CustomInput
                                                                            ...
                                                                            label={
                                                                                <React.Fragment>
                                                                                    <span className="d-block h6 mt-1 mb-3">{input.label}</span>
                                                                                    <span className="text-muted text-sm">{input.text}</span>
                                                                                </React.Fragment>}
                                                                    ...
                                                                    :
                                                                    <FormGroup key={input.name} className={`col-md-${input.col ? input.col : 6}`}>
                                                                        <Label className="form-label" for={input.name}>{input.label}</Label>
                                                                        <Input type={input.type} name={input.name} placeholder={input.placeholder} value={formInputs[block.name] && formInputs[block.name][tab.name] && formInputs[block.name][tab.name][input.name] || ''} // Checked input control 
            ...
            <div className="my-5 d-flex justify-content-between flex-column flex-lg-row">
                <Link href={prev[1]} passHref>
                    <Button color="link" className="text-muted"><i className="fa fa-angle-left mr-2" />{prev[0]}</Button>
                </Link>
                <Link href={next[1]} passHref>
                    <Button color="dark">{next[0]}<i className="fa fa-angle-right ml-2" /></Button>
                </Link>
            </div>
