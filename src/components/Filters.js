...
import { Col, Collapse, FormGroup, Form, CustomInput, Label, Input } from 'reactstrap'
...
const Filters = ({ top }) => {
        ...
        <Form className={top ? '' : 'mt-4 mt-lg-0'}>
            {data.map(item =>
                <FormGroup key={item.value} className="mb-1">
                    <CustomInput type="checkbox" name={item.name} id={item.value} label={<React.Fragment>{item.label} <small>({item.productcount})</small></React.Fragment>} checked={filterInputs[item.name] ? filterInputs[item.name].includes(item.value) : ''} onChange={(e) => onInputChange(e)} />
                </FormGroup>
            )}
        </Form>
    ...
    const ColorsFilter = () =>
        <ul className={`list-inline mb-0 colours-wrapper ${top ? '' : 'mt-4 mt-lg-0'}`}>
            {colors.map(color =>
                <li key={color.value} className="list-inline-item">
                    <Label className={`btn-colour ${filterInputs["colour"] && filterInputs["colour"].some(item => item === color.value) ? 'active' : ''}`} for={color.value} style={{ backgroundColor: color.color }} />
                    <Input className="input-invisible" type="checkbox" name="colour" id={color.value} checked={filterInputs["colour"] ? filterInputs["colour"].includes(color.value) : ''} onChange={(e) => onInputChange(e)} />
                </li>
            )}
        </ul>
        ...
        <Form className={top ? '' : 'mt-4 mt-lg-0'}>
            {sizes.map(size =>
                <FormGroup key={size.value} className="mb-1">
                    <CustomInput type="radio" name="size" id={size.value} label={size.name} checked={filterInputs["size"] ? filterInputs["size"] === size.value : ''} onChange={(e) => onInputChange(e)} />
                </FormGroup>
            )}
        </Form>
    ...
    return (
        filtersBlocks.map((filter, index) =>
            top ? // If top filters position
                ...
                <div key={index} className="sidebar-block px-3 px-lg-0">
                    ...
                    {/* COLLAPSE TOGGLE */}
                    <a className="d-lg-none block-toggler" data-toggle="collapse" aria-expanded={collapse[filter.subtitle]} onClick={() => toggleCollapse(filter.subtitle)}>{filter.title}<span className="block-toggler-icon" />
                    </a>
                    {/* END COLLAPSE TOGGLE */}
